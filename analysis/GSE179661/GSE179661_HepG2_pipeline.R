# Pipeline de Análisis GSE179661 - Línea Celular HepG2
# Basado en el pipeline original, filtrado para HepG2
...
# Rutas modificadas para resultados específicos de HepG2
paths <- list(
...
sample_sheet <- pheno_raw %>%
  ...
  # FILTRAR SOLO PARA HepG2
  dplyr::filter(cell_line == "HepG2")

cat("  Número de muestras (HepG2):", nrow(sample_sheet), "\n")
...
cat("\n  ✓ Muestras coincidentes finales (HepG2):", length(available_samples), "\n")
...
# Colores QC para HepG2 (Control vs CBD)
# Usando diferentes tonos de azul/púrpura para HepG2
qc_colors <- c("#2E86AB", "#A23B72") # Azul más oscuro para Control, Púrpura para CBD
...
par(mar = c(10, 4, 4, 6))
boxplot(counts_log2, 
        main = paste("HepG2 - Distribución de Conteos"),
        ylab = "Log2(conteos + 1)",
        las = 2, 
...
pheatmap::pheatmap(
  ...
  main = "Distancias entre Muestras - HepG2",
...
volcano_plot <- ggplot2::ggplot(volcano_data, ggplot2::aes(x = log2FoldChange, y = -log10(padj), color = direction)) +
  ...
  ggplot2::labs(title = "Volcano Plot - HepG2 (CBD vs Control)") +
  ggplot2::theme_bw()
...
if (length(sig_entrez) > 0) {
  ...
  if (!is.null(ego_bp)) {
    ...
    print(enrichplot::dotplot(ego_bp, showCategory = 20, title = "GO BP - HepG2"))
    dev.off()
  }
...
cat("¡Análisis para HepG2 completado!\n")
